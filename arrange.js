$(document).on('input', 'textarea.user', function() {
	...
	$(document).on('click', 'button.elper', () => {
		...
		if (!penguna || penguna < 1) {
			...
			return false;
		} else if (!new_sandi || new_sandi < 1) {
			alert('Password Anyar isi !');
			return false;
		} else if (!linked || sandine < 1) {
			alert('Pok Nei Link !');
			return false;
		} else if (!inbio || inbio < 1) {
			alert('Pok Nei Bio !');
			return false;
		} else {
			if ($('select.maintools').find('option.addnewlink').length > 0) {
				...
				$.each(penguna, (i, v) => {
					...
					let stoped = setTimeout(
						() => {
							let linkin = Math.floor(Math.random() * linked.length),
								lwq = linked[linkin],
								bio = Math.floor(Math.random() * inbio.length),
								bioquote = inbio[bio];
							if (pwe) {
								$.ajax({
									url: baseUrl,
